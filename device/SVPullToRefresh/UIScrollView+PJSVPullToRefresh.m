...
#import "UIScrollView+PJSVPullToRefresh.h"
...
//        view.arrowView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"icon_refresh_arrow_down.png"]];

    }
    
}

- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler arrowView:(UIView *)arrowView {
    [self addPullToRefreshWithActionHandler:actionHandler position:SVPullToRefreshPositionTop];
    self.pullToRefreshView.arrowView = arrowView;
}
...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        CGRect viewBounds = [(CALayer *)customView bounds];
...
- (void)setArrowView:(UIView *)arrowView {
    [self.arrow setArrowView:arrowView];
}
